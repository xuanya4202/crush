...
int crush_calc_straw(struct crush_map *map, struct crush_bucket_straw *bucket)
{
	...
	reverse = (int *)malloc(sizeof(int) * size);
        if (!reverse)
                return -1;
	if (size)
		reverse[0] = 0;
...
{
    struct crush_map cmap = {0};
    cmap.straw_calc_version = 1;

    struct crush_bucket_straw host = {0};
    host.h.size = 10;
    host.item_weights = (__u32*)malloc(sizeof(unsigned int)*10);
    host.h.items = (__s32*)malloc(sizeof(__s32)*10);
    host.straws = (__u32*)malloc(sizeof(unsigned int)*10);
    int i = 0;
    for(i = 0; i < 10; i++)
    {
    ...
    crush_calc_straw(&cmap, &host);

    int out[1000] = {0};
    for(i = 0; i < 1000; i++)
    {
        out[i] = bucket_straw_choose(&host, pg[i], 3);
    }

    host.item_weights[1] = 0.5*(float)0x10000;
    ...
    for(i = 0; i < 1000; i++)
    {
        out1[i] = bucket_straw_choose(&host, pg[i], 3);
    }
